...
import com.andryyu.helper.sub.R;
import com.andryyu.helper.sub.http.HttpKnife;
import com.andryyu.helper.sub.http.HttpLog;
import com.andryyu.helper.sub.http.Response;
...
public class GithubImpl implements Github {
    ...
    public GithubImpl(Context context) {
        ...
        authError = new AuthError("GithubToken 失效");
    }
    ...
    public String createToken(String username, String password)
        ...
        GithubToken token = new Gson().fromJson(response.body(), GithubToken.class);
    ...
    public String findCertainTokenID(String username, String password)
        ...
        Gson gson = new Gson();
        ArrayList<GithubToken> tokenList = gson.fromJson(response.body(),
                new TypeToken<List<GithubToken>>() {
                }.getType());
        System.out.println("listToken gson = " + tokenList);
        for (int i = 0; i < tokenList.size(); i++) {
            GithubToken token = tokenList.get(i);
